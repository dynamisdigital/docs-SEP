...
- **Roteiros de teste manual**: migrados para o repositório irmão **`sep-test-app`** (antes em `docs-sep/roteiros-teste/`). Lá ficam o hub `CENARIOS-TESTE-JORNADAS-USUARIO.md`, os `ROTEIRO-NN-*.md` e o mini-app web (`app/`), agora com persistência em arquivo (`data/db.json`) via servidor local. Ver o `README.md` do `sep-test-app`.
...
- Para validar o produto manualmente, use o repositório **`sep-test-app`**: rode `npm start` e abra `http://127.0.0.1:4599`, seguindo o `ROTEIRO-00` antes de qualquer jornada. Depois de editar um roteiro `.md`, rode `npm run gerar` para o app refletir a mudança.
