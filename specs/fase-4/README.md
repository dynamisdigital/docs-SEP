...
| 37 | [`037-sprint-37-normalizacao-taxonomia-erro.md`](./037-sprint-37-normalizacao-taxonomia-erro.md) | Normalizar a taxonomia: decidir **o que o prefixo significa** e **qual e a convencao de sufixo**, resolver as colisoes e re-prefixar `credores` — **correcao de divida**; **MERGEADA develop+main em 2026-09-10** (PR #110/#111; 10 commits; particao **144 = 143 publicados + 1 excluido**, `AUTH-403-001` inalcancavel; 2297 -> 2318 testes; registro `PrefixoCodigoErro` e gate no build; resultado na §Resultado da spec). Decisoes de 2026-09-10 ([ADR 0020](../../adr/0020-convencao-codigos-de-erro.md): prefixo = area funcional com o `credito` saindo de `CRD` para `PRP`, sufixo **numerico**, publicacao na propria sprint — **zero mudanca de contrato**; as duas recomendacoes originais cairam porque renomeariam codigos ja publicados; steps em [`037`](../../steps-fase-4/backend/037-sprint-37-steps.md)). **Preve ADR**, ao contrario da 036: define convencao que vincula sprints futuras e atravessa os tres repos. Nao e sprint de rename — e sprint de decisao. Achados que a dimensionam: o prefixo **nao e identificador de modulo** (um modulo com dois prefixos, um prefixo em dois modulos, e nenhum registro em lugar nenhum); `credores` ocupa a faixa `CRD` com **35 codigos contra 10** do `credito`, que lhe da nome; das 12 colisoes, **2 nao sao colisao** (constante duplicada com mesmo significado, corrige por deduplicacao); e o `PIX` tem **28 sufixos semanticos contra 3 numericos**, entao e convencao paralela com dono, nao desvio. Consome o numero 37 e provoca o **5o recuo** do backend da Fase 5, para 38-41 | 8 |
| 38 | [`038-sprint-38-modulo-notificacao-historico.md`](./038-sprint-38-modulo-notificacao-historico.md) | Modulo `notificacao` transversal, historico persistido (`V61`), canal `IN_APP` e **um** gatilho real (`PixTransferenciaConcluidaEvent` -> tomador) — **produto novo**; **MERGEADA develop+main** (2026-09-14, PR #112/#113; ADR 0021; 2318 -> 2434 testes); planejada em 2026-09-01. **Preve ADR** (supersede parcial do 0014). Frente **A** do levantamento de notificacoes: medido, o `sep-api` tem **71 eventos de dominio e tres pontos de envio**, e os tres falam com o tomador em momento ruim (regua de cobranca, renegociacao, conta bloqueada) — o produto **so fala com o tomador para cobrar**. Ha **duas infra paralelas**: a completa presa dentro de `cobranca`, e a rasa (`shared.email.EmailService`, 1 consumidor) que e absorvida aqui. **Nao ha historico nem opt-out.** Consome o numero 38 e provoca o **6o recuo** do backend da Fase 5, para 39-42 | 8 |
...
| F-27 | [`127-fsprint-27-central-notificacao-web.md`](./127-fsprint-27-central-notificacao-web.md) | Primeira superficie de notificacao do `sep-app`: contador de nao-lidas no shell autenticado, lista paginada, marcar como lida e mock MSW fiel — **produto novo**; **MERGEADA develop+main** (2026-09-14, PR #170/#171, arvore `b8e6012`). Lado web da frente **A**. **Depende da 038 em `develop`**; independente da M-19 e da cadeia P1. **Sem polling e sem tempo real** por decisao: oferece `read your writes` no contador, nao `read others writes`. Nao provoca recuo (a Fase 5 nao tem sprint de web) | 6 |
| F-28 | [`128-fsprint-28-contract-check-erro-typecheck-web.md`](./128-fsprint-28-contract-check-erro-typecheck-web.md) | Fechar os dois pontos cegos do `contract-check.mjs` que impediam gatear o catalogo de codigos de erro (**corpo de erro por status** via `errorResponses`, **pertinencia de enum** via `enumSubset` opt-in), executar a Task 126.3 que a F-26 deixou pendente e instalar `typecheck:spec` no CI — **correcao de divida**; **MERGEADA develop+main** (PR #151/#152, 2026-09-10). Vitest 855/97 -> **875/97**, `contract:check` 85/0, `typecheck:spec` novo no CI (inclui os specs do Playwright); 24 mutacoes mortas. Declara **dois** codigos, nao tres: o `verify-totp` so ramifica `MFA-400-003/004`. O gate morde **na renovacao do snapshot**, nao na mudanca do backend. "Obrigatorio de um lado so" fica **nao verificavel**, medido. Pontos cegos (i)-(iv) da F-24 remedidos e mantidos fora | 6 |
