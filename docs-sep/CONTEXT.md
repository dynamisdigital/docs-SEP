...
- **Simplificacao do fluxo GitHub (2026-05-06)** — apos os incidentes acumulados (PRs #10/#11 com squash-merge errado, branches `develop` apagadas pelo `delete_branch_on_merge`, working tree dirty viajando entre branches via `git switch`, regras de fluxo fragmentadas em 5 arquivos de memoria do agente), o fluxo Git dos repos de codigo foi simplificado. **Decisoes consolidadas** (inspiradas nas regras enxutas do plan Sprint 1 do projeto pilot-broker):
  - **Branch model**: `develop` eliminada nos 3 repos (`sep-api`, `sep-app`, `sep-mobile`); todas as branches de trabalho passam a nascer direto de `main`. Nome padrao: `feature/<sprint-ou-tema>` (ou `feature/fix-<descricao>` para bug em codigo ja mergeado).
  ...
  - **Hook PostToolUse `git add` automatico**: removido de `sep-api/.claude/settings.json`, `sep-app/.claude/settings.json`, `sep-mobile/.claude/settings.json`. Agente passa a fazer `git add <paths-especificos>` explicito antes de cada commit.
  - **Bugs em codigo ja entregue**: criar nova branch `feature/fix-<descricao>` a partir de `main`. Regra anterior ("reusar a branch da sprint") so vale enquanto a branch ainda nao foi mergeada.
  - **Setting `delete_branch_on_merge=true`** mantido (revertendo recomendacao temporaria do incidente PRs #10/#11) — alinha com fluxo "branch some apos merge, nova branch nasce de main fresca". Forensics via `git reflog` no clone local + Files changed do PR antes do merge.
  - **Procedimento de cleanup working tree dirty** removido como regra explicita — substituido pelo principio "nao fazer `git switch` com mudancas pendentes; comitar a Task antes de trocar de branch".
  ...
  - **`docs-SEP` continua 100% manual** (sem mudanca): agente edita working tree, NAO comita, NAO push.
  - **AGENT.md** atualizado com novo fluxo (working tree apenas — usuario revisa e comita manualmente).
  - **Memoria do agente**: 5 arquivos `feedback_*.md` consolidados em 1 (`feedback_git_workflow.md`); manteve-se `feedback_docs_sep_manual.md`, `feedback_bugfix_mesma_branch.md` (atualizada) e `feedback_springdoc_compat.md`.

## Proximo passo mais natural

Com Sprint 0/F-Sprint 0/M-Sprint 0 (2026-05-04), Sprints 1, 2, 3 e 4 backend + M-Sprint 1 mobile (2026-05-05/2026-05-06) concluidas, e fluxo GitHub simplificado (2026-05-06), os proximos passos provaveis sao:
- usuario abrir o PR da branch `feature/msprint-1-tokens-notion` no `sep-mobile` (push e PR sao manuais por design); apos merge em `main`, branch remota apaga sozinha e branch local permanece como historico ate decisao manual
