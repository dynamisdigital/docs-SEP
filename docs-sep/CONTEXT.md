...
Com Sprint 0/F-Sprint 0/M-Sprint 0 (2026-05-04), Sprint 1 (2026-05-05) e Sprint 2 (2026-05-05) concluidas, os proximos passos provaveis sao:
- abrir PR de `feature/sprint-2-gestao-usuarios` -> `develop` no `sep-api`; aguardar CI verde + merge
- desligar setting `delete_branch_on_merge` do repo `sep-api` (e checar/desligar nos repos `sep-app` e `sep-mobile`) para evitar repetir incidente de branches sumidas
- gerar `docs-SEP/steps/backend/003-sprint-3-steps.md` just-in-time antes de executar a Sprint 3 (Seguranca e Autenticacao JWT). Sprint 3 consome `Usuario`, `UsuarioCreateDto`, `UsuarioSenhaUpdateDto` (declarado mas ainda sem use case), `CriarUsuarioUseCase` da Sprint 2; entrega login, JWT issuer/validator, `/auth/me`, autorizacao por ownership, alteracao da propria senha autenticada. Spec [`specs/fase-1/003-sprint-3-seguranca-autenticacao.md`](../specs/fase-1/003-sprint-3-seguranca-autenticacao.md)
- iniciar F-Sprint 2 (telas Apple publicas) consumindo contratos reais de `POST /api/v1/usuarios` (deixar MSW so para erros): spec [`specs/fase-1/102-fsprint-2-telas-apple-publicas.md`](../specs/fase-1/102-fsprint-2-telas-apple-publicas.md), steps a criar
