...
| 35 | Follow-up / correcao de divida | Allowlist de proxy (`forward-headers-strategy`), validacao de `LockoutProperties` no boot, `405` faltante, config e codigo morto, `Clock` injetavel e itens de contrato | [`035`](../specs/fase-4/035-sprint-35-divida-config-lockout-contrato.md) | **planejada** (2026-08-05). E a proxima sprint de backend; comecar pelo Gate 35.0. **Atencao**: a Task 35.5 planeja remover `ContaBloqueadaException.CODIGO` como codigo morto, e a Sprint 36 lhe da consumidor — a 35.5 deve manter apenas a metade do `countByIpAndJanela` |
