...
| 36 | Produto novo / contrato de erro | Publicar a taxonomia de codigos de erro no fio: `codigo` no `ErrorResponseDto`, propagacao no ponto unico `build()` (16 handlers, 17 depois da 35) e catalogo do **subconjunto apto** no OpenAPI, com **perimetro** sobre colisoes e formato divergente | [`036`](../specs/fase-4/036-sprint-36-codigos-erro-no-fio.md) | **CONCLUIDA na branch** `feature/sprint-36-codigos-erro` em **2026-09-08**; push e PR pendentes (manuais). **2262 -> 2297 testes / 0 falhas / 368 classes**, `clean build` e `spotlessCheck` verdes, 8 commits. **O Gate 36.0 derrubou sete numeros ou premissas da spec**: a taxonomia e **133** codigos em **13** prefixos (nao ~103 em 12); as colisoes sao **16** (nao 12); as violacoes de formato **31** (nao 12); os `private` **26** (nao 12); os handlers sem codigo **13 de 17** (nao 10 de 16); os dois orfaos `BOF-*` **sao alcancaveis**; e **`build()` nao e o ponto unico de montagem** — sao cinco construcoes, quatro delas em filtros e entry points do Spring Security, entao `401`/`403`/`429` da cadeia de seguranca **seguem sem codigo**, por decisao declarada. Publicados **80**, excluidos **53**, particao recalculada a cada `./gradlew build`. **A sprint achou um defeito nela mesma**: a Task 36.2 vazava codigos fora do perimetro para o fio (`CRD-403-001`, excluido por colisao, chegava ao corpo e violava o `enum` publicado); corrigido na 36.6. **26 mutacoes, 26 mortas.** Desbloqueia a F-26 e a M-18. Detalhe em [`SPRINT-36-PR.md`](../repos/sep-api/SPRINT-36-PR.md) e [`CODIGOS-DE-ERRO.md`](../repos/sep-api/CODIGOS-DE-ERRO.md) |
