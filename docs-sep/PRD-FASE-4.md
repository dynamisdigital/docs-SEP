...
| 36 | Produto novo / contrato de erro | Publicar a taxonomia de codigos de erro no fio: `codigo` no `ErrorResponseDto`, propagacao no ponto unico `build()` (16 handlers, 17 depois da 35) e catalogo do **subconjunto apto** no OpenAPI, com **perimetro** sobre colisoes e formato divergente | [`036`](../specs/fase-4/036-sprint-36-codigos-erro-no-fio.md) | **planejada** (2026-09-01), **revisada no mesmo dia**. Recomendacao **P1** do [`DIAGNOSTICO-PRODUTO.md`](./DIAGNOSTICO-PRODUTO.md): a taxonomia e construida no dominio e **descartada na fronteira HTTP** — `getCodigo()` tem **zero** consumidores em `src/main`. A revisao mediu **~103 codigos em 12 prefixos** (nao 67 em 9), **12 definidos duas vezes** com significados diferentes, **12 violacoes de formato** (nao 1) e **3 excecoes orfas** de `DomainException` (nao 1) — dai o perimetro. Fecha, com a F-26 e a M-18, seis itens que o `STATE.md` hoje lista como defeitos separados. **Depende da 35 em `develop`** (mesmo arquivo, nao mesmo contrato) e **desbloqueia** a F-26 e a M-18. Provocou o **4o recuo** do backend da Fase 5 — mecanismo **encerrado em 2026-09-01** pela faixa por fase ([`../AGENT.md`](../AGENT.md) §Numeracao); a Fase 5 e fixa em 50-99 e nao recua mais |
