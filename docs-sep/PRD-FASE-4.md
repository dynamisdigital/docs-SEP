...
Mapa da fase com o status corrente de cada sprint (atualizado em 2026-08-05: web **concluido**,
incluindo o par corretivo de lockout (Sprint 33 + F-21), a **F-22 mergeada exceto a Task 6** e a
**F-23**, que executou essa Task 6 como sprint propria e **esgota o recorte web da Fase 4**; no
backend a **Sprint 34 esta mergeada em develop+main**, quitando a divida que o par corretivo
registrou; no mobile a **M-17 esta mergeada**, e restam M-14/M-15, bloqueadas por gate externo de
hardware macOS). Cada
...
(**28 arquivos**, incluindo a `120` criada para o Gate F-18.0, o par corretivo `033`/`121`, o trio de divida `034`/`122`/`217`, a `123` criada em 2026-08-05 para a Task F-22.6 retomada, e a **cadeia P1** `036`/`126`/`218`, criada em 2026-09-01 a partir do [`DIAGNOSTICO-PRODUTO.md`](./DIAGNOSTICO-PRODUTO.md)); os **steps** continuam **just-in-time**
...
| 34 | Follow-up / correcao de divida | Follow-ups do lockout (observabilidade da tentativa barrada, `Retry-After`, invariante de config no startup, evicção do `RateLimiterRegistry`, politica de lockout no contrato) + as 5 lacunas de OpenAPI registradas como `knownGaps` pela F-19 | [`034`](../specs/fase-4/034-sprint-34-followups-lockout-contrato.md) | concluida (PR #103 develop `0d24602` / #104 main `550fed3`, 2026-08-03; gate de contrato no `sep-app` via PR #120/#121, `contract:check` de 29 lacunas para 1; 13 commits, **2220 testes / 0 falhas**, migration `V60`, sem ADR. Tentativa contra conta bloqueada passa a deixar rastro com tipo de audit proprio; `detalhes` do audit serializado; `Retry-After` no `423` com o restante real e no `429` com o periodo de refresh; invariante `rate-limit > max-attempts` validada no boot (e os defaults do POJO, que a violavam, corrigidos); mapa de limitadores com teto LRU e origem limitada a 45 chars; `GET /api/v1/auth/politica-lockout` publico; `X-Step-Up-Token` declarado nos 24 endpoints por `OperationCustomizer`, enums de contrato/assinatura publicados e headers de resposta documentados, tudo travado por regressao. **Duas premissas da spec caiam**: o `Duration` do dashboard ja era corretamente documentado como `string` — a divergencia e do `sep-app` —, entao esse `knownGap` **nao fecha aqui**. **Nao entrega escopo novo**) |
| 35 | Follow-up / correcao de divida | Allowlist de proxy (`forward-headers-strategy`), validacao de `LockoutProperties` no boot, `405` faltante, config e codigo morto, `Clock` injetavel, itens de contrato e o `400` de path variable | [`035`](../specs/fase-4/035-sprint-35-divida-config-lockout-contrato.md) | **concluida** (PR #105 develop `23004b9` / #106 main `8cabf2c`, 2026-09-02; `develop` == `main` por conteudo e arvore byte-identica a da branch conferida **depois** do back-merge). 16 commits, **2262 testes / 0 falhas / 363 classes** (partida 2220/355), sem migration e sem ADR. 8 Tasks: os steps erraram o escopo em duas (a 35.2 exigiu mudar codigo alem da config, porque o `RemoteIpValve` ignora mas nao remove o header; a 35.6 mediu 10 literais de MDC onde a spec previa 4) e o teste mostrou antes do codigo nas duas. **46 mutacoes, cinco sobreviveram** — inclusive um bean de `ModelResolver` sem `openapi31` que apagava 21 `description` e 17 `example` do OpenAPI passando verde em 2258 testes. Desbloqueia a **F-24.5**. **Nao entrega escopo novo** |
| 36 | Produto novo / contrato de erro | Publicar a taxonomia de codigos de erro no fio: `codigo` no `ErrorResponseDto`, propagacao no ponto unico `build()` (16 handlers, 17 depois da 35) e catalogo do **subconjunto apto** no OpenAPI, com **perimetro** sobre colisoes e formato divergente | [`036`](../specs/fase-4/036-sprint-36-codigos-erro-no-fio.md) | **MERGEADA develop+main em 2026-09-08** — PR **#107** (squash `452a09f`, back-merge `a774aa4`) e PR **#108** (`042949b`); `develop` == `main` por conteudo, arvore byte-identica a da branch que passou nos gates. **2262 -> 2297 testes / 0 falhas / 368 classes**, `clean build` e `spotlessCheck` verdes, 8 commits. **O Gate 36.0 derrubou sete numeros ou premissas da spec**: a taxonomia e **133** codigos em **13** prefixos (nao ~103 em 12); as colisoes sao **16** (nao 12); as violacoes de formato **31** (nao 12); os `private` **26** (nao 12); os handlers sem codigo **13 de 17** (nao 10 de 16); os dois orfaos `BOF-*` **sao alcancaveis**; e **`build()` nao e o ponto unico de montagem** — sao cinco construcoes, quatro delas em filtros e entry points do Spring Security, entao `401`/`403`/`429` da cadeia de seguranca **seguem sem codigo**, por decisao declarada. Publicados **80**, excluidos **53**, particao recalculada a cada `./gradlew build`. **A sprint achou um defeito nela mesma**: a Task 36.2 vazava codigos fora do perimetro para o fio (`CRD-403-001`, excluido por colisao, chegava ao corpo e violava o `enum` publicado); corrigido na 36.6. **26 mutacoes, 26 mortas.** Desbloqueia a F-26 e a M-18. Detalhe em [`SPRINT-36-PR.md`](../repos/sep-api/SPRINT-36-PR.md) e [`CODIGOS-DE-ERRO.md`](../repos/sep-api/CODIGOS-DE-ERRO.md) |
| 37 | Correcao de divida / convencao | Normalizar a taxonomia de erro: decidir **o que o prefixo significa** e **qual e a convencao de sufixo**, deduplicar condicoes de significado identico, separar as colisoes reais e re-prefixar o `credito` (`CRD` -> `PRP`) | [`037`](../specs/fase-4/037-sprint-37-normalizacao-taxonomia-erro.md) | **MERGEADA develop+main em 2026-09-10** (PR #110 squash `c8b98ae` / #111 `5fe83a1`, conferida por conteudo). [ADR 0020](../adr/0020-convencao-codigos-de-erro.md): prefixo = area funcional, sufixo numerico, publicacao na propria sprint — **zero mudanca de contrato**; as duas recomendacoes originais da spec cairam na medicao porque renomeariam codigos publicados. Particao **133 = 80 + 53 -> 144 = 143 + 1** (o unico excluido e `AUTH-403-001`, inalcancavel); registro `PrefixoCodigoErro` e gate de convencao no build; os 143 congelados contra rename. Testes **2297 -> 2318**. Provocou o **5o recuo** do backend da Fase 5 — mecanismo **encerrado em 2026-09-01** pela faixa por fase; a Fase 5 e fixa em 50-99 |
| 38 | Produto novo / notificacao | Modulo `notificacao` transversal, historico persistido (migration `V61`), canal `IN_APP` e **um** gatilho real (`PixTransferenciaConcluidaEvent` -> tomador) | [`038`](../specs/fase-4/038-sprint-38-modulo-notificacao-historico.md) | **MERGEADA develop+main** (2026-09-14, PR #112/#113, conferida por conteudo); [ADR 0021](../adr/0021-modulo-notificacao-transversal.md) aceito. Resultado em [`NOTIFICACOES.md`](../repos/sep-api/NOTIFICACOES.md) e nos steps. Planejada em 2026-09-01. Frente **A** do levantamento de notificacoes. **Preve ADR** (supersede parcial do [0014](../adr/0014-estrategia-de-notificacoes-transacionais.md)). Medido: **71 eventos de dominio, tres pontos de envio**, os tres para o tomador em momento ruim — o produto **so fala com o tomador para cobrar divida ou avisar que perdeu acesso**. Duas infra paralelas (a completa presa em `cobranca`, a rasa em `shared.email`); **sem historico, sem opt-out**. Absorve o `EmailService` (1 consumidor) e **nao** migra a regua de cobranca — duplicacao temporaria declarada. **Desbloqueia** a F-27 e a M-19. Provocou o **6o e ultimo recuo** do backend da Fase 5 — a faixa por fase, fixada em 2026-09-01, encerrou o mecanismo; a Fase 5 vive em 50-99 |
...
| F-22 | Follow-up / correcao de divida | Contrato de erro verificavel no `contract:check` (status de erro declarados + deteccao de `knownGap` obsoleto) e follow-ups da F-21: `verify-totp` por status, foco em `access-denied`, landmarks, remocao do registro orfao, helper unico de erro; Task final consome a Sprint 34 | [`122`](../specs/fase-4/122-fsprint-22-contrato-erro-followups-web.md) | concluida **exceto a Task 6** (PR #116, 2026-07-31; `contract:check` passa a validar status de erro declarados em `erros` — 9 operacoes —, `responseHeaders` vira mapa por status e `knownGap` obsoleto falha com exit 1 contra o snapshot versionado; `verify-totp` traduz erro por status usando o corpo no `400`, que colapsa tres causas; foco em `access-denied`, landmarks em `verify-totp`/`redirect-to-app`, `RegisterComponent` orfao removido, extracao de mensagem unificada em `core/api/`. Vitest 745, Playwright 38, `contract:check` 84 operacoes/29 lacunas, audit 0; snapshot **nao** renovado. **Task 6 nao executada, mas destravada em 2026-08-03**: a Sprint 34 foi mergeada (PR #103/#104) e o snapshot foi regenerado (PR #120/#121), entao os dois bloqueios cairam. **Executada pela F-23**. **Nao entrega escopo novo**) |
| F-23 | Follow-up / correcao de divida | Consumir `GET /auth/politica-lockout` e o header `Retry-After`: a jornada de conta bloqueada para de anunciar numeros fixos no codigo (retomada da Task F-22.6 como sprint propria) | [`123`](../specs/fase-4/123-fsprint-23-politica-lockout-web.md) | concluida (PR #125 develop squash `9fb9788` / #126 main `b2809b3`, 2026-08-05; `develop` == `main` por conteudo, back-merge `c72b393` vazio). `/account-locked` deriva a copy dos tres numeros efetivos do ambiente e **continua funcional sem a chamada** — ela e destino de redirect e alcancavel por URL direta; `authInterceptor` isenta o endpoint publico, fechando um caminho em que o token velho levava `401` e o `errorInterceptor` **arrancava o usuario da pagina** (a consequencia registrada antes, "cai de volta no texto fixo", estava subestimada); login usa o `Retry-After` no `423`/`429`, onde o header ganha do corpo porque a `message` do backend anuncia a duracao nominal por design. Vitest 765, Playwright 39, `contract:check` 85 operacoes/1 lacuna (a do `Duration`, intencional). 26 mutacoes verificadas. **Gate declarado pendente**: smoke real contra `:8080` nao executado, entao o CORS do `Retry-After` e um eventual `429` na propria pagina seguem sem prova — o MSW nao consegue provar nenhum dos dois. **Esgota o recorte web da Fase 4**; nao entrega escopo novo |
| F-24 | Follow-up / correcao de divida | Divida tecnica do web: os dois defeitos vivos que a F-22 e a F-23 deixaram, descriptor de contrato e duplicacao de helper de teste | [`124`](../specs/fase-4/124-fsprint-24-divida-tecnica-web.md) | concluida (PR #130 develop `69d5165` / PR #131 main `3b09a3c`, 2026-08-06; back-merge `b821496`; `develop` == `main` por diff de conteudo, reconferido no Gate F-25.0 em 2026-08-21). 13 commits. Sem tela, endpoint, DTO, migration ou regra nova. **`contract:check` sai de 1 lacuna para ZERO** (85 operacoes) — primeira vez desde a criacao do gate na F-19 (2026-07-16) —, e o zero nao e vazio: mutar o tipo de volta reprova e o `varrerGapsObsoletos` reprova o gap sobrevivente, entao corrigir o tipo e remover o `knownGap` sao obrigatoriamente o mesmo commit. Vitest 765/93 -> **802/94**; Playwright 39; `lint`, `lint:scss`, `format:check`, `build` verdes. **36 mutacoes** aplicadas e mortas; definicoes de helper de teste **80 -> 2** (reconferido: hoje ha exatamente duas, ambas em `src/testing/estabilizar.ts`). **Os dois defeitos vivos fecharam**: o `errorInterceptor` deixou de arrancar o usuario da `/account-locked` (a F-23 isentara `/auth/politica-lockout` so no `authInterceptor`, o que impede o header de ser ENVIADO mas nao a resposta de ser TRATADA), e o KPI do dashboard parou de renderizar `NaNmin` (`tempoMedioResolucao30d` era `number` enquanto o backend serializa `Duration` como ISO-8601, e o mock devolvia `7200` — **mais correto que o servidor**, e por isso nenhum teste via). **Gate declarado pendente**: smoke real contra `:8080` nao executado. Quatro pontos cegos do `contract-check.mjs` medidos e nenhum corrigido — sprint propria |
| F-25 | Produto novo / transparencia LGPD | Aviso de cookies dispensavel e pagina publica de politica de privacidade, descrevendo o armazenamento que o web de fato usa | [`125`](../specs/fase-4/125-fsprint-25-aviso-cookies-privacidade-web.md) | **MERGEADA develop+main** (PR #136/#137, 2026-08-21, conferida por conteudo na varredura de 2026-09-02; branch `feature/fsprint-25-aviso-cookies`, 7 commits). **Transparencia, nao consentimento**: medido, o produto emite **um** cookie (`sep-refresh`, de autenticacao) e nao ha script de terceiro nem biblioteca de rastreamento, entao opt-in gatearia zero cookies — sem "recusar", sem categorias, botao "Entendi", e a politica **afirma** a ausencia de opcao de recusa em vez de omiti-la. O aceite nunca vai ao servidor. Vitest 802/94 -> **833/97**, Playwright 39/11 -> **42/12**, `contract:check` **85/0 inalterado** (a sprint nao consome contrato), `audit` **1 high -> 0**. O Gate F-25.0 derrubou a baseline do `audit` nos dois sentidos e revelou que **o gate de CI instalado pela D-1 estava vermelho em `develop` sem ninguem saber**. A medicao dos e2e achou **defeito de produto, nao artefato de teste**: a faixa `position: fixed` cobria o ultimo elemento de qualquer pagina e interceptava o clique em "Iniciar onboarding" — o `storageState` que os proprios steps prescreviam teria escondido o defeito; a faixa passou a reservar a propria altura e **nenhuma blindagem foi aplicada**. 25 mutacoes distintas em 33 aplicacoes; **tres testes reescritos por terem sobrevivido**. **Gates declarados pendentes**: revisao juridica do texto (marcador visivel na pagina) e configuracao real do cookie em producao, nao observavel aqui. **Expoe e nao corrige**: `SEP_ACCESS_TOKEN` guarda JWT de acesso em `localStorage` (exige ADR) |
| F-26 | Produto novo / contrato de erro | Consumir o `codigo` publicado pela Sprint 36: helper `codigoDeErroDaApi()` em `core/api/api-error.ts`, catalogo gateado no `contract:check` e discriminacao do `400` **colapsado** do `verify-totp` pelos codigos `MFA-400-002`/`003`/`004` | [`126`](../specs/fase-4/126-fsprint-26-consumo-codigos-erro-web.md) | **MERGEADA develop+main** (PR #145/#146, conferida em 2026-09-09; branch `feature/fsprint-26-codigos-erro`, 5 commits; os testes duplicados pelo back-merge `11bd729` sairam no fix #149/#150). Lado web da recomendacao **P1**. O caso escolhido e o que a **propria F-24 declarou indiscriminavel**: o docblock de `copy-de-erro.ts` registra que o `400` do `verify-totp` colapsa tres causas e *"so o `message` as discrimina"* — com a 36 elas passam a ter codigo estavel. **Os 3 literais byte-identicos que o `STATE.md` cita ja foram fechados pela F-24** (viraram `copy-de-erro.ts`); o que sobrou e o **ramo**, nao a frase. **Depende da 036 e da F-25 em `develop`** — as duas conferidas por conteudo no Gate F-26.0. Fora de escopo declarado: os 78 pontos de ramificacao por status. **Resultado medido**: Vitest **833 -> 855 / 97**, Playwright **42**, `contract:check` **85 / 0**, `audit` 0, demais gates verdes. **14 mutacoes distintas, 12 mortas**; as 2 sobreviventes tem causa declarada (nada constroi um `ApiErrorResponse` e specs nao sao typechecados; o snapshot depende da 126.3). **A Task 126.3 NAO foi executada**: o `contract-check.mjs` nao expressa corpo de erro (`verificarCorpoDaResposta` itera so `sucesso`) nem pertinencia de enum (`verificarEnum` exige igualdade de conjunto — provado por sonda) — **quinto e sexto pontos cegos**, alem dos quatro da F-24; o catalogo fica **sem gate automatico**, declarado. **Smoke real executado**: `POST /auth/totp/verify` com challenge invalido devolve `"codigo":"MFA-400-004"` no fio. **A §Ancora 2 da spec caiu**: codigo em branco e bean validation (`@NotBlank`) e volta **sem** `codigo`, entao `MFA-400-002` so e alcancavel com challenge valido — vale para a **M-18**. Baseline exigiu PR proprio (**#143**): `format:check` e `npm audit` estavam **vermelhos em `develop`**. Detalhe em [`SPRINT-F-26-PR.md`](../repos/sep-app/SPRINT-F-26-PR.md) |
| F-27 | Produto novo / notificacao | Primeira superficie de notificacao do `sep-app`: contador de nao-lidas no shell autenticado, lista paginada, marcar como lida e mock MSW fiel | [`127`](../specs/fase-4/127-fsprint-27-central-notificacao-web.md) | **MERGEADA develop+main** (PR #170/#171, 2026-09-14, arvore `b8e6012` conferida por conteudo; 12 commits, com dois P2 do review de fim de sprint corrigidos antes do merge; Vitest 875/97 -> 951/100, Playwright 42 -> 48, `contract:check` 85 -> 88 / 0, smoke real `:8080` 19/19). Planejada em 2026-09-01. Lado web da frente **A**. **Dependia da 038 em `develop`**; independente da M-19 e da cadeia P1. **Sem polling e sem tempo real por decisao**: oferece `read your writes` no contador (quem marca ve cair na hora), nao `read others writes`. A central nasce quase vazia — com um gatilho, so tomador com desembolso concluido tem o que ver, entao o mock **precisa** cobrir o caso vazio. Nao provoca recuo (a Fase 5 nao tem sprint de web; e desde 2026-09-01 nenhuma sprint de Fase 4 provoca recuo) |
| F-28 | Follow-up / divida de tooling e contrato | Gate de corpo de erro no `contract:check` (`errorResponses` e `enumSubset`) e typecheck dos specs no CI, fechando os follow-ups (m) e (n) da F-26 | [`128`](../specs/fase-4/128-fsprint-28-contract-check-erro-typecheck-web.md) | **MERGEADA develop+main** (PR #151 squash `15ed341` / #152 `49f568a`, 2026-09-10, conferida por conteudo; 10 commits). O catalogo de codigos consumido pelo `verify-totp` passa a ter gate, que morde **na renovacao do snapshot**; `typecheck:spec` no CI cobrindo tambem o Playwright. Vitest 855/97 -> **875/97**, `contract:check` **85 / 0**, 24 mutacoes mortas. Follow-ups (u)-(z) mergeados em 2026-09-11 (#156/#158, #157/#159), com o workflow `CONTRACT-DRIFT` diario contra o runtime do `sep-api` `develop`. Esta linha faltava na tabela desde o planejamento |
